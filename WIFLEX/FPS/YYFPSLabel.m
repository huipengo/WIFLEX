...
+ (void)showFPSInView:(UIView *)view frame:(CGRect)frame {
    if (!view) {
        view = [UIApplication sharedApplication].keyWindow;
    }
    if (CGRectEqualToRect(frame, CGRectZero)) {
        CGFloat fps_y = 15.0 + 44.0 + [[UIApplication sharedApplication] statusBarFrame].size.height;
        frame = CGRectMake([UIScreen mainScreen].bounds.size.width - 60.0, fps_y, 50.0, 30.0);
    }
    YYFPSLabel *fpsLabel = [view viewWithTag:wbFPSLabelTag];
    if (!fpsLabel) {
        fpsLabel = [[YYFPSLabel alloc] initWithFrame:frame];
    }
    else {
